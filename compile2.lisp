  ...
  "Compile the expression x into a list of instructions"
    (cond
      ...
      ((case (first x)
         ...
         (FN-SET!  (arg-count x 3)
                 (assert (symbolp (second x)) (x)
                         "Only symbols can be fn-set!, not ~a in ~a"
                         (second x) x)
                 (seq (comp (fourth x) env t t)
                      (gen-set (cadr x) env)
                      (if (not val?) (gen 'POP))
                      (unless more? (gen 'RETURN))))         (IF     (arg-count x 2 3)
...
(defun comp-lambda (args body env &optional name)
...
(defun new-fn (&key code env name args)
  ...
  (assemble (make-fn :env env :name name :args args
                     :code (clvm-optimize code))))

(defun clvm-optimize (code) code)
